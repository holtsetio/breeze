...
export class Leaf {
    ...
    buildGeometry() {
        ...
        return;
        this.object = new THREE.Mesh(geometry, Leaf.material);
    ...
    static async createInstances() {
        const geometry = new THREE.InstancedBufferGeometry().copy(Leaf.objects[0].geometry);
        const vertexOffsetArray = new Uint32Array(Leaf.objects.length);
        for (let i = 0; i < Leaf.objects.length; i++) {
            vertexOffsetArray[i] = Leaf.objects[i].verletVertices[0][0].id;
        }
        ...
        geometry.instanceCount = Leaf.objects.length;
        Leaf.object = new THREE.Mesh(geometry, Leaf.material);
        Leaf.object.count = Leaf.objects.length;
        Leaf.object.frustumCulled = false;
        Leaf.object.castShadow = true;
        Leaf.object.receiveShadow = true;
        console.log(Leaf.object);
    }
